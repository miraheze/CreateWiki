...
class ManageInactiveWikisV2 extends Maintenance {
	...
	private function checkLastActivity(
		...
		if (!$wiki->isClosed()) {
			...
			$isInactive = $wiki->isInactive() && $wiki->getInactiveTimestamp();
			if ($isInactive && $wiki->getInactiveTimestamp() < date('YmdHis', strtotime("-{$closeDays} days"))) {
				if ($canWrite) {
					$wiki->markClosed();
					$this->notify($dbName);
				...
				$this->handleInactiveWiki($dbName, $wiki, $lastActivityTimestamp, $closeDays, $canWrite);
			}
		} else {
			// Handle already closed wikis
			$this->handleClosedWiki($dbName, $wiki, $removeDays, $canWrite);
		}
	...
	private function handleClosedWiki(
		...
		$isClosed = $wiki->isClosed() && $wiki->getClosedTimestamp();
		if ($isClosed && $wiki->getClosedTimestamp() < date('YmdHis', strtotime("-{$removeDays} days"))) {
			if ($canWrite) {
